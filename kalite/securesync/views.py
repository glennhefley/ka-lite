...
#, facility_required, facility_from_request
...
def login(request, facility):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect(form.non_field_errors() or request.next or reverse("coach_reports" if form.get_user().is_teacher else "homepage"))
        else:
